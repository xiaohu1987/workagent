...
type QuickNote = { id: string; title: string; content: string; updatedAt: string; cloudNoteId: string | null };
        ...
        <div className="quick-notes-layout">
          <aside className="quick-notes-list">
            ...
            {notes.length ? <div className="quick-notes-items">{notes.map((note) => <button key={note.id} type="button" className={`quick-notes-item ${note.id === selectedId ? "selected" : ""}`} onClick={() => onSelect(note)} onContextMenu={(event) => { event.preventDefault(); onOpenMenu({ x: event.clientX, y: event.clientY, note }); }}>
              ...
              <span className="quick-notes-item-meta">{note.cloudNoteId ? <span className="quick-notes-item-cloud">已关联云笔记</span> : null}<span className="quick-notes-item-time">{new Date(note.updatedAt).toLocaleString()}</span></span>
            </button>)}</div> : <div className="quick-notes-empty">新建一条笔记，记录随时浮现的想法。</div>}
          </aside>
          <div className="quick-notes-editor"><div className="quick-notes-markdown-bar" aria-label="文档编辑器"><strong>文档</strong><span>富文本编辑 · 右键插入内容</span></div><textarea className="quick-notes-content-input quick-notes-plain-editor" value={content} onChange={(event) => onContentChange(event.target.value)} placeholder="在这里开始写作..." spellCheck={false} /><footer className="quick-notes-footer"><span className={`quick-notes-status ${status.includes("已同步") ? "is-synced" : ""}`}>{status}</span><div className="quick-notes-footer-actions">{cloudNoteId ? <span className="quick-notes-cloud-tag" title="该笔记已关联云笔记，两侧修改会立即互相同步">已关联云笔记</span> : null}<button type="button" className="button secondary" disabled={saving || syncingCloud || !selectedId} onClick={() => void onSyncToCloud()}>{syncingCloud ? "同步中..." : "同步到云笔记"}</button><button type="button" className="button primary" disabled={saving} onClick={() => void onSave()}>{saving ? "保存中..." : "保存"}</button></div></footer></div>
        </div>
