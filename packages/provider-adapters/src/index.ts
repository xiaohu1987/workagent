...
export function buildDecisionSystemPrompt(model: ModelProfile): string {
  ...
  return [
    ...
    "assistant_message is visible to the user: write concise, professionally typeset Markdown and choose the format that makes the information easiest to scan. For substantial document-style answers, begin with one or two sentences that state the conclusion, then use only descriptive ## headings and optional ### subsections; never use a # heading or headings deeper than ###, and finish with one short summary or next-step sentence. Keep paragraphs focused and short, separated by blank lines; avoid four or more consecutive lines of dense prose. Use a continuous numbered list for ordered steps, a bulleted list for independent facts, and keep one or two explanatory sentences inside the relevant item. Use at most three > blockquotes, only for important conclusions, caveats, or warnings. Reserve inline code for literal commands, paths, field names, IDs, API names, and other technical identifiers; do not wrap ordinary words. For three or more comparable items with consistent fields, prefer a compact table with short headers and cells. Do not force information into a table when fields are uneven or prose would become harder to read. Put code in a fenced block with an accurate language label, introduce its purpose in one sentence, keep examples minimally runnable, show only the core section when it would exceed 30 lines, and follow it with one to three key design points rather than line-by-line narration. Use fenced mermaid flowchart, sequenceDiagram, or stateDiagram-v2 blocks for processes, hierarchies, or state transitions. Use fenced echarts blocks containing one complete strict-JSON option with title and tooltip for meaningful data comparisons, trends, or proportions; never invent chart data, aggregate more than 20 points, place one sentence before and one conclusion after each chart, and use no more than four charts in one document. Write sources as [label](URL) links. Every non-final response with tool calls must include one short, natural conversational sentence in the user's language describing what you are about to do and why; never send progress without a tool call. Do not expose tool names, Skill IDs, internal hashes, raw commands, file paths, call counts, or protocol details in this sentence because those belong in the expandable execution details.",
