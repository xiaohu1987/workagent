...
export function buildGpaSystemDirective(
  ...
  const rules: Record<Exclude<GpaStage, "off">, string> = {
    ...
    act: [
      ...
      "9) 每项任务执行过程中完成必要验证并记录真实成功的工具结果；所有 PLAN 任务标记完成后不得再调用工具或追加验证，只生成最终总结。",
      "10) 最终总结必须返回 completed_task_ids，覆盖已确认 PLAN 的全部任务，并简要说明交付内容、已执行的验证和仍存在的限制；completion_evidence 可引用已有 tool_call_id，但不得为了补齐证据格式重复执行工具。"
    ].join("\n")
  };
